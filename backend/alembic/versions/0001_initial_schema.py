...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "account_type IN ('individual', 'multiempresa')", name="chk_account_type"
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "firewall_command_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("firewall_commands.id"),
            nullable=False,
            unique=True,
        ),
        sa.Column(
    ...
    op.create_index(
        "idx_alert_deliveries_alert_channel_id", "alert_deliveries", ["alert_channel_id"]
    )
    op.create_index(
